...
// TODO: 1. Remember threadId of each D, V, C and check if it matches and if there is only one when a new element is added
// TODO: 2. concurrent access (+ What happens if two threads try to CREATE the same mutex at the same time)
// TODO: 3. check that we cannot do something bad with returned Semaphore object (for example, override its max counter to make non-binary) if yes then replace Semaphore with an own class derived from Semaphore, but with restrictions
// TODO: May be rename this to MutexManager
public abstract class AbstractMutexManager <TypeKey, TypeValue> extends HashMap {
    ...
    protected final Class <TypeValue> TypeV = (Class<TypeValue>) (
        ...
        @Override @Deprecated public void putAll(Map m) {
            throw new UnsupportedOperationException(
                    "This method is deprecated, use clearRecordsForThread() instead."
            );
        }
        ...
        // Careful about types:
        // https://stackoverflow.com/questions/33765010/put-an-object-of-the-wrong-type-in-a-map.
        protected TypeValue insert(TypeKey key, TypeValue value) {

            if (super.containsKey(key)) {
                // NOTE: may be it has been just created in a sibling thread,
                // but the value still was not assigned?
                // No, because we organized key names so that neither two
                // threads can build the same hash key
                // Even more. When thread A asks thread B it uses not the same key when
                // thread B asks thread A.

                // class cast exception?
                TypeValue val = (TypeValue) super.get(key);
                if (val != value) {
                ...
                try {
                    ...
                    Application.printMsg("ConcurrentModificationException on writing to hash[" + key + "]");
                    throw e;
                }
            }
        ...
        protected TypeValue obtain(TypeKey key) throws NullPointerException, NoSuchElementException, TypeNotPresentException, ClassCastException {
            ...
            // Will in this case (or even in case we just call containsKey()
            // with another type and variables are not equal literally as int and String)
            // some exception be thrown?
            if (super.containsKey(key)) {
                ...
                if (TypeV.isInstance(value)) {
                    ...
                    throw new TypeNotPresentException(
                            "Wrong map element type: expected " + TypeV.getClass().toString() +
                            ", got " + value.getClass() + ".", null
                    );
                }
            } else {
